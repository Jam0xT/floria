...
export default Object.freeze({
	...
	'heal': {
		/*
			heal,		// 回血量 单位: 点
			ready_time,	// 准备时间 从 load 到可以开始判定
			pend_time,	// 判定时间 从开始判定到回血
		*/
		'onLoad': [
			function (instance) {
				const sv = instance.var.skill_var;
				sv.ready = 0; // 初始化 ready 计时器
				sv.pend = 0; // 初始化 pend 计时器
			}
		],
		'onTick': [
			function (instance) {
				const sv = instance.var.skill_var;
				if ( sv.ready < sv.ready_time ) { // 准备时间
					sv.ready += 1;
					return ;
				}
				// 准备完成
